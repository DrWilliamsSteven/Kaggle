...
matplotlib.use("Agg")  # CHECK: What does this do?
...
def preprocessing(_args=args):
    ...
    imagePaths = sorted(list(paths.list_images(_args["dataset"])))
    random.seed(42)
...
def convertLabels(split_data):
    ...
    # vector)
    lb = LabelBinarizer()
    trainY_vector = lb.fit_transform(trainY)
    testY_vector = lb.transform(testY)
...
def saveModelToDisk(model, lb, _args=args):
    # save the model and label binarizer to disk
    print("[INFO] serializing network and label binarizer...")
    model.save(_args["model"])
    f = open(_args["label_bin"], "wb")
